    ...
    def forward_features(self, x):
        ...
        for i, (embed, drop, blocks, pos_blk) in enumerate(
            ...
            for j, blk in enumerate(blocks):
                ...
                if j == 9:
                    xblock_3 = x.reshape(B, *size, -1).permute(0, 3, 1, 2).contiguous()
            if i < len(self.depths) - 1:
                x = x.reshape(B, *size, -1).permute(0, 3, 1, 2).contiguous()

        x = self.norm(x)
        # return x.mean(dim=1)  # GAP here
        return xblock_3,x.view(x.size(0),512,7,7)
...
def _create_twins(variant, pretrained=False, **kwargs):
    ...
    if pretrained:
        print("twins loading pre weight...")
...
# model = twins_svt_small(pretrained=False)
# print(model)
# # print(list(model.named_modules())[160:250])
# # print(model.named_children())
# model.named_modules()

# new_model = nn.Sequential(*list(model.children())[2:4])
# print(new_model)
...
# a = torch.rand(2,196,256).permute(0,2,1)
# rr = nn.Sequential(
#     nn.Conv1d(in_channels=256,out_channels=512,kernel_size=3,stride=2),
#     nn.BatchNorm1d(512),
#     nn.ReLU()
# )
#
